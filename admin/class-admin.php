...
class Admin
{
	...
	protected static function assistant_steps()
	{
		return array(
			...
			'faqs' => array('number' => 5, 'title' => __('FAQs', 'ai-knowledge'), 'description' => __('Edita las preguntas frecuentes públicas que se publican como documento propio y se enlazan desde llms.txt. En sitios multidioma se conserva una FAQ independiente por idioma.', 'ai-knowledge')),
	...
	protected static function assistant_screen_content($step)
	{
		...
		<?php elseif ('content' === $step) : $post_types = get_post_types(array('public' => true), 'objects'); ?>
			<div class="wookb-assistant-field"><span><?php esc_html_e('Modo de selección', 'ai-knowledge'); ?></span><label><input type="radio" name="post_types_mode" value="explicit" <?php checked('explicit', $settings['post_types_mode']); ?> /> <?php esc_html_e('Solo los tipos marcados', 'ai-knowledge'); ?></label><label><input type="radio" name="post_types_mode" value="all_public" <?php checked('all_public', $settings['post_types_mode']); ?> /> <?php esc_html_e('Todos los tipos públicos', 'ai-knowledge'); ?></label></div><div class="wookb-chip-group"><?php foreach ($post_types as $post_type) : if ('attachment' === $post_type->name) continue; ?><label class="wookb-chip"><input type="checkbox" name="post_types[]" value="<?php echo esc_attr($post_type->name); ?>" <?php checked(in_array($post_type->name, (array) $settings['post_types'], true)); ?> /> <?php echo esc_html($post_type->labels->name); ?></label><?php endforeach; ?></div><p><a target="_blank" rel="noopener noreferrer" href="<?php echo esc_url(admin_url('admin.php?page=ai-knowledge&tab=contenido')); ?>"><?php esc_html_e('Abrir configuración avanzada de contenido', 'ai-knowledge'); ?></a></p>
		...
		<?php elseif ('faqs' === $step) : $langs = Wpml::active_languages(); $lang = isset($_POST['faq_lang']) ? sanitize_key(wp_unslash($_POST['faq_lang'])) : ($langs ? $langs[0] : 'es'); if (!in_array($lang, $langs, true)) $lang = $langs ? $langs[0] : 'es'; ?>
			<div class="wookb-assistant-field"><span><?php esc_html_e('Idioma de la FAQ', 'ai-knowledge'); ?></span><select name="faq_lang"><?php foreach ($langs ?: array('es') as $faq_lang) : ?><option value="<?php echo esc_attr($faq_lang); ?>" <?php selected($faq_lang, $lang); ?>><?php echo esc_html(strtoupper($faq_lang)); ?></option><?php endforeach; ?></select></div><label class="wookb-assistant-field"><span><?php esc_html_e('Preguntas frecuentes en Markdown', 'ai-knowledge'); ?></span><textarea name="llms_faq" rows="12" placeholder="### ¿Puedo devolver un producto?&#10;Respuesta..."><?php echo esc_textarea(Llms_Faq::read($lang)); ?></textarea><small><?php esc_html_e('Contenido público enlazado desde llms.txt. Puedes editarlo manualmente y ampliarlo después desde la pantalla completa de FAQs.', 'ai-knowledge'); ?></small></label><p><a target="_blank" rel="noopener noreferrer" href="<?php echo esc_url(admin_url('admin.php?page=ai-knowledge&tab=faqs&lang=' . $lang)); ?>"><?php esc_html_e('Abrir generación y edición avanzada de FAQs', 'ai-knowledge'); ?></a></p>
	...
	protected static function assistant_save_step($step)
	{
		...
		if ('ai' === $step) {
			...
			if ('business' === $step) Chatbot_Prompt_Builder::write_info_doc();
			if ('chatbot' === $step) Scope::update_settings(array('chatbot_docs_list_limit' => max(0, (int) ($_POST['chatbot_docs_list_limit'] ?? Chatbot_Relevance_Guard::DOCS_LIST_LIMIT_DEFAULT)))); // phpcs:ignore
		} elseif ('faqs' === $step) {
			$langs = Wpml::active_languages();
			$lang = isset($_POST['faq_lang']) ? sanitize_key(wp_unslash($_POST['faq_lang'])) : ($langs ? $langs[0] : 'es'); // phpcs:ignore
			if (!in_array($lang, $langs, true)) $lang = $langs ? $langs[0] : 'es';
