...
class Llms_Txt {
	...
	public static function render() {
		...
		$info_path = Markdown_Store::base_dir() . '/info.md';
		...
		// encabezados es ruido y confunde sobre el idioma real del documento).
		$show_lang_suffix = count( array_unique( wp_list_pluck( $rows, 'lang' ) ) ) > 1;
		$main_lang        = Languages::main_language();
		...
		foreach ( $groups as $category => $by_lang ) {
			...
			foreach ( $by_lang as $lang => $items ) {
				$lines[] = '## ' . $category . ( $show_lang_suffix && (string) $lang !== (string) $main_lang ? ' (' . strtoupper( $lang ) . ')' : '' );
	...
	/**
	 * Marca que identifica un llms.txt físico creado por este plugin (uninstall.php
	 * solo borra el archivo de la raíz si la lleva o contiene la firma histórica
	 * del enlace a openapi.json). Va al final, como comentario HTML.
	 */
	const PHYSICAL_MARKER = '<!-- Generated by AI Knowledge & Visibility -->';
